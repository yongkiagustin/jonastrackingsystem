...
public class EyesTracker extends Tracker<Face> {
    ...
    public void onUpdate(Detector.Detections<Face> detections, Face face) {
        if (face.getIsLeftEyeOpenProbability() > THRESHOLD || face.getIsRightEyeOpenProbability() > THRESHOLD) {
            ...
            ((MapsActivity)context).updateMainView(Condition.USER_EYES_OPEN);
        }
        else {
            Log.i(TAG, "onUpdate: Close Eyes Detected");
            ((MapsActivity)context).updateMainView(Condition.USER_EYES_CLOSED);
        }
    }
    ...
    public void onMissing(Detector.Detections<Face> detections) {
        ...
        ((MapsActivity)context).updateMainView(Condition.FACE_NOT_FOUND);
    }
